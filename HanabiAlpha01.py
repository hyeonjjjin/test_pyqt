...
GiveHintAlpha = uic.loadUiType("testUI01.ui")[0]
#MainBoard
#giveHint
#etc..
...
class AppDropDeck(QWidget): #카드 내기 창
    def __init__(self):
        QWidget.__init__(self)
    def paintEvent(self, e):
        ...
        space = QLabel(" ")
        btnfin = QPushButton("선택")
        btnfin.setCheckable(False)


        rbtn0 = QRadioButton(self)
        ...
        layoutrbtn.addWidget(rbtn0)
        layoutrbtn.addWidget(rbtn1)
        layoutrbtn.addWidget(rbtn2)
        layoutrbtn.addWidget(rbtn3)
        ...
        btnfin.setMaximumWidth(140)

        #위젯 배치
        layoutSpace = QHBoxLayout()
        layoutSpace.addWidget(space)
        layoutSpace.addWidget(btnfin)
        layoutSpace.addWidget(space)
        ...
        throwDeck = 0
        if btnfin.isChecked()==True:
            if rbtn0.isChecked():
                throwDeck = playerDeck[0]
            elif rbtn1.isChecked():
                throwDeck = playerDeck[1]
            elif rbtn2.isChecked():
                throwDeck = playerDeck[2]
            elif rbtn3.isChecked():
                throwDeck = playerDeck[3]
            print(throwDeck)
